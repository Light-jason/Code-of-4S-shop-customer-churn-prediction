% RNN for 二分类任务（递归神经网络）% RNN for classification
...
if(nargin == 5)
    ...
    opts.momentum = 0.9; 
    opts.training_object = 1e-2;     % 设置训练目标 % set the train object
    opts.batch_size = ceil(size(y_train,1)/40); 
    opts.T = 9;   % 这里的T是向上取特征的最大个数,相当于RNN_for_4S中T=4的情形
                  % ‘T’ represents the max number of historical warranty features 
end
